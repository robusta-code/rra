...
import io.robusta.rra.controller.ControllerException;
...
public class JaxRsController {
    ...
    public Response getBasicAuthentificationResponse() {
        if (!isSecure(getUriInfo())) {
            return response.status(401).entity("<a href='http://docs.oracle.com/javaee/5/tutorial/doc/bnbxw.html'>Establishing a Secure Connection Using SSL</a>").build();
        } else
    ...
    public boolean isTablet() {
        return getUserAgent().get(0).toUpperCase().contains("TABLET") || getUserAgent().get(0).toUpperCase().contains("IPAD");
    }
